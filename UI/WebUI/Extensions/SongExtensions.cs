...
namespace SetGenerator.WebUI.Extensions
{
    public static class SongExtensions
    {
        public static SetList GenerateSets(this IList<Song> songs, SetListDetail setlistDetail, int bandId, long userId)
        {
            ...
            if (songs != null)
            {
                var defaultsingerid = songs[0].Band.DefaultSingerId;
                setlist = new SetList
                {
                    BandId = bandId,
                ...
                for (var setnum = 1; setnum <= numsets; setnum++)
                {
                    ...
                    for (var song = 1; song <= numsongs; song++)
                    {
                        ...
                        {
                            lastsingerid = lastOrDefault.Song.SingerId ?? 0;
                        ...
                        if (lastsingerid == nextsong.SingerId)
                            singercount++;
                        else
                            singercount = 1;
        ...
        private static bool IsSameInstrumentation(IEnumerable<SongMemberInstrument> lastInstrumentation, IEnumerable<SongMemberInstrument> newInstrumentation)
        {
            ...
            var dicNew = newInstrumentation.ToDictionary(x => x.MemberId, x => x.InstrumentId);
            foreach (var ilast in lastInstrumentation.Where(ilast => ilast.InstrumentId != dicNew[ilast.MemberId]))
            {
                returnval = false;
            }
        ...
        private static int[] GetValidIds(ICollection<Song> songs, IEnumerable<Set> setsongs, int instrcount, IEnumerable<SongMemberInstrument> lastinstruments)
        {
            ...
            var usedSongs = setsongs.Select(x => x.SongId);

            if (lastinstruments != null)
            {
                var dicInstruments = lastinstruments.ToDictionary(x => x.MemberId, x => x.InstrumentId);
                if (instrcount < Constants.MinSongInstrumentationCount)
                {
                    foreach (var s in songs)
                    {
                        ...
                        foreach (
                            var smi in
                                s.SongMemberInstruments.Where(smi => smi.InstrumentId != dicInstruments[smi.MemberId]))
                        {
                            isadd = false;
                        }
                        if (isadd)
