...
namespace SetGenerator.WebUI.Common
{
    public class CommonSong
    {
        ...
        public IList<BandMemberDetail> GetBandMemberDetails(int bandId)
        {
            ...
            var members = _memberRepository.GetList(bandId);
            ...
            foreach (var m in members)
            {
                ...
                foreach (var smid in m.MemberInstruments.Select(i => new BandMemberInstrumentDetail { Id = i.InstrumentId, Name = i.Instrument.Name, Abbreviation = i.Instrument.Abbreviation }))
                {
        ...
        {
            var keys = _keyRepository.GetList();
        ...
        public ICollection<TableColumnDetail> GetTableColumnList(IEnumerable<UserPreferenceTableColumn> columns, IEnumerable<UserPreferenceTableMember> members, int userTableId, int bandId)
        {
            ...
            list.AddRange(columns
                .Where(x => x.TableColumn.TableId == userTableId)
